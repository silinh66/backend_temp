...
const getPrice = async (symbol) => {
  ...
  try {
    for (let i = 0; i < 170; i++) {
      ...
      let url = `http://localhost:3020/DailyOhlc?symbol=${symbol}&fromDate=${fromDate}&toDate=${toDate}`;
    ...
    let bPrices = listSymbol
      ? listSymbol.map((item, index) => {
          return {
            ...
            close: item.Close,
            volume: item.Volume,
            timestamp: moment(item.TradingDate, "DD/MM/YYYY").format("X"),
          };
        })
      : [];
    ...
    fs.writeFileSync(`./logs/${symbol}.json`, JSON.stringify(bPrices));
    let bPricesMap = bPrices.map((item) => {
      return [
        ...
        item.close,
        +item.volume,
        +item.timestamp + 86400,
      ];
    });
    if (bPricesMap.length > 0) {
      //delete all symbol === symbol in history_1d
      await query("DELETE FROM history_1d WHERE symbol = ?", [symbol]);
      //insert new symbol
      await query(
        "INSERT INTO history_1d (symbol, date, open, high, low, close, volume, timestamp) VALUES ?",
        [bPricesMap]
      );
    }
  } catch (error) {
